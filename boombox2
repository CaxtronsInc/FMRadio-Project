...
unsigned char LCD_SEG_BOOL = 0;
int LCD_SEG_DISP;
unsigned char LCD_SEG_ONE;
int LCD_SEG_DEC;
...
unsigned int fave_stations[] = {875, 881, 903, 964, 1046, 1080};
...
rom unsigned char digit[11]={0xFC,0x60,0xDA,0xF2,0x66,0xB6,0xBE,0xE0,0xFE,0xF6,0x00};
...
{ 
	...
	unsigned int rotA=CTRL_ROTA;
	unsigned int rotB=CTRL_ROTB;
	...
	while(volChange==0){ 
		...
		if (PIR1bits.TMR1IF)
		{
			if (LCD_SEG_BOOL)
			{
				LCDdisplay (LCD_SEG_DISP, LCD_SEG_DEC);
				LCD_SEG_BOOL = 0;
			}
			else
			{
				SEG_DEC = 0;
				SEG_ONE = 0;
				serial(digit[10], digit[10], digit[10]);
				LCD_SEG_BOOL = 1;
			}
			PIR1bits.TMR1IF = 0;
			TMR1H = 0b10000000;                      // Clear timer count
   		 	TMR1L = 0;
		}
...
//************************************************************************************************  
		...
		{ 
	        if (ASk_SSK_down) 			
			{
				bounce_count++;                          // pressed state detected 
...
//	  
//	    if (PRESET_6 && (bounce_count == DetectsInARow))  
//	    { 
//	        hold_count = 0; 
//	    	do{
...
{	unsigned char data6[]="     Muted     ";
	...
	i=0;line(0xd0);
	while(data6[i]!='\0'){dis_data(data6[i]);i++;}} 
	else{
	if(mute_test==1)
...
void Init() {
	...
	CMCON = 0x07;
	TRISA = 0b00000010;             // RA0 and RA1 pbutton
	TRISB = 0b00110111;				// RB0 and RB5 pbutton
	TRISC = 0b01011000;				// RC3 and RC4 do the I2C bus
	TRISD = 0b00000000;				
	TRISE = 0b00000000;				
	TRISF = 0b11111110;
...
{	
	...
	unsigned char data5[]="    Volume:     ";
	
//	cleardt();Delay_ms(10);i=0;
//	while(data0[i]!='\0'){dis_data(data0[i]);Delay_ms(30);i++;}	
//	 i=0;line(0xc0);
//	while(data1[i]!='\0'){dis_data(data1[i]);Delay_ms(30);i++;}
//	 i=0;line(0x90);
//	while(data2[i]!='\0'){dis_data(data2[i]);Delay_ms(30);i++;}
//	 i=0;line(0xd0);
//	while(data3[i]!='\0'){dis_data(data3[i]);Delay_ms(30);i++;}
//	Delay_ms(3000);i=0;
//	while(i<85){dis_data(0x20);Delay_ms(30);i++;}	
//	cleardt();
	i=0;
	while(data4[i]!='\0'){dis_data(data4[i]);Delay_ms(1);i++;}
	 i=0;line(0x90);
	while(data5[i]!='\0'){dis_data(data5[i]);Delay_ms(1);i++;}		
}
...
{
	lcd_data1 = ((cmdout>>4)&0x01);
	lcd_data2 = ((cmdout>>5)&0x01);
	lcd_data3 = ((cmdout>>6)&0x01);
	lcd_data4 = ((cmdout>>7)&0x01);
	rs=0;
	en=1;
	Delay10TCYx(1);
	//Delay_ms(1);
...
{
	lcd_data1 = ((dataout>>4)&0x01);
	lcd_data2 = ((dataout>>5)&0x01);
	lcd_data3 = ((dataout>>6)&0x01);
	lcd_data4 = ((dataout>>7)&0x01);
	rs=1;
	en=1;
	Delay10TCYx(1);
	//Delay_ms(1);
...
/*************************************************************************************
**	LCDdisplay() is a function that takes an integer value to display and a 1 or 0 for
**	the display of the decimal place. For example, an input of LCDdisplay(895, 1) would 
**	be display 89.5 on screen.
**************************************************************************************/
void LCDdisplay (int freqInput, int deci)
{
	int sz=0;
	int freq[5] = {0,0,0,0,0};	//The value is placed in here in reverse order, as digits 0 to 9.
	
	IntToArray(freqInput, freq, &sz);
	serial(digit[freq[2]], digit[freq[1]], digit[freq[0]]);
	if (deci == 1) SEG_DEC = 1;
		else SEG_DEC = 0;
	if (freq[sz]==1) 
	{
...
void serial(char arrA, char arrB, char arrC){
	int j = 0;
	unsigned char temp;
	...
	{
		temp = ((arrA>>j) & 0x01);
		SEG_DAT3 = ((arrC>>j) & 0x01);
		SEG_DAT1 = ((arrA>>j) & 0x01);
		SEG_DAT2 = ((arrB>>j) & 0x01);
...
/****	Function to take integer input and return the individual***
*****	digits to an array along with the size of the array of	***
*****	digits in order to cycle through and use later.		*******/

void IntToArray(int input, int freq[], int *sz){
	int q = 0, r = 0, i=0;
	if( input>=1000 ) freq[3] = 1;
	do{
		r = input % 10;
		q = input / 10;
		input = input / 10;
